...
NODE_TYPES: dict[str, dict] = {
    ...
    "schedule": {
        ...
        **_make_node("Agendamento (Cron)", "trigger", "Inicia o fluxo em horario agendado.", [], _run_schedule,
            [{"key": "cron", "label": "Cron Expression", "type": "text",
              "placeholder": "Ex: 0 9 * * * (todo dia as 9h)", "default": "0 9 * * *",
              "help": "Padrao: todos os dias as 9h. Altere a expressao Cron para outro horario."}]),
    },
    "wait_until_message": {
    ...
    "ai": {
        ...
        **_make_node("IA (Gerar resposta)", "ai", "Chama o LLM da empresa para gerar uma resposta.", [], _run_ai,
            [{"key": "prompt", "label": "Prompt", "type": "textarea",
              ...
              "default": "Responda de forma clara e cordial: {{ data.message.text }}",
              "help": "Padrao: usa a mensagem recebida. Altere para definir o tom ou a tarefa da IA."},
             {"key": "history", "label": "Usar memoria da conversa", "type": "toggle", "default": True,
              "help": "Padrao: ligado, para a IA considerar mensagens anteriores da conversa."},
             {"key": "system_prompt", "label": "System prompt (opcional)", "type": "textarea",
              "placeholder": "Deixe vazio para usar o da empresa.", "default": "",
              "help": "Padrao: usa o System Prompt definido em Configuracao > IA. Preencha somente para sobrescrever neste node."}]),
    },
    "ai_rag": {
        "type": "ai_rag",
        **_make_node("IA RAG (Base de Conhecimento)", "ai", "Busca contexto na base de conhecimento e gera resposta com IA.", [], run_rag_node,
            [{"key": "prompt", "label": "Pergunta / Prompt", "type": "textarea",
              "placeholder": "Ex: {{ data.message.text }}", "default": "{{ data.message.text }}",
              "help": "Padrao: pesquisa a mensagem recebida na Base de Conhecimento."},
             {"key": "top_k", "label": "No. de contextos relevantes", "type": "number", "default": 5,
              "help": "Padrao: 5 trechos. Reduza para respostas mais objetivas ou aumente para buscar mais contexto."},
             {"key": "system_prompt", "label": "System prompt (opcional)", "type": "textarea", "default": "",
              "help": "Padrao: usa a instrucao da empresa. Preencha somente para uma regra especifica deste node."}]),
    },
    "set": {
        "type": "set",
        **_make_node("Definir variavel", "data", "Guarda um valor em uma variavel do fluxo.", [], _run_set,
            [{"key": "variable", "label": "Nome da variavel", "type": "text", "default": "resultado",
              "help": "Padrao: resultado. Troque pelo nome que os proximos nodes devem usar, sem espacos."},
             {"key": "value", "label": "Valor", "type": "text",
              "placeholder": "Valor literal ou {{ data.campo }}", "default": "{{ data.message.text }}",
              "help": "Padrao: salva a mensagem recebida. Use texto fixo ou outra variavel do contexto."}]),
    },
    "code": {
        "type": "code",
    ...
    "condition": {
        ...
        **_make_node("Condicao", "logic", "Desvia o fluxo conforme uma condicao (saidas true/false).", [], _run_condition,
            [{"key": "value", "label": "Variavel", "type": "text",
              "placeholder": "data.mensagem.texto", "default": "data.message.text",
              "help": "Padrao: verifica a mensagem recebida. Informe outro caminho do contexto para mudar a regra."},
             {"key": "operator", "label": "Operador", "type": "select",
              "options": ["==", "!=", "contains", "not_contains", ">", "<", ">=", "<=", "is_true", "is_empty", "is_not_empty"], "default": "is_not_empty",
    ...
    "delay": {
        ...
        **_make_node("Aguardar (Delay)", "logic", "Pausa o fluxo por alguns segundos.", [], _run_delay,
            [{"key": "seconds", "label": "Segundos", "type": "number", "default": 1,
              "help": "Padrao: 1 segundo. Aumente somente quando o proximo passo realmente precisar esperar."}]),
    },
    "http": {
        "type": "http",
    ...
    "log": {
        ...
        **_make_node("Registrar (Log)", "data", "Escreve uma mensagem no log da execucao.", [], _run_log,
            [{"key": "message", "label": "Mensagem", "type": "textarea", "default": "Fluxo executado.",
              "help": "Padrao: registra que o fluxo foi executado. Altere para incluir a informacao que deseja diagnosticar."}]),
    },
    "execute_workflow": {
        "type": "execute_workflow",
        **_make_node("Executar Workflow", "core", "Chama outro workflow (sub-workflow reutilizavel).", [], _run_execute_workflow,
            [{"key": "workflow_id", "label": "ID do Workflow", "type": "number", "placeholder": "Ex: 12",
              "requires_explicit_value": True,
              "help": "Selecione o ID de outro workflow da mesma empresa. Nao ha padrao seguro porque cada empresa possui workflows diferentes."}]),
    },
}
